...
public class Model {
	...
	private void cerca2(Set<Esame> parziale, int L, int m) {
		...
		if(sommaCrediti > m) { //soluzione non valida
			return;
		} else if (sommaCrediti == m) { //soluzione valida, controlliamo se è la migliore fino a qui
		...
		//se arriviamo qui, sicuramente i crediti sono < m 
		if(L == esami.size()) {
			return;
		}
	...
	private void cerca1(Set<Esame> parziale, int L, int m) {
		...
		//se siamo qui, generiamo sotto-problemi
		for(Esame e: esami) {
